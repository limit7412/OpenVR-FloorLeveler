...
(`StandingZeroPoseToRawTrackingPose`) を書き換えることで行う。SteamVR 上のすべての
アプリケーション (VRChat を含む) は standing 座標を基準とするため、補正は全アプリに
透過的に反映される。
...
1. **接続** — SteamVR を起動した状態で `FloorLeveler.exe` を実行し、「接続 / 再試行」を押す
   - 接続時に、現在の Chaperone 設定が自動でバックアップされる
...
3. **サンプリング** — 床の複数点を記録する。方式は 3 つ
   ...
   | 連続方式 | 床に接地させたまま引きずると一定間隔で自動記録される (既定 100 ms) |

   デバイス原点ではなく**接地点**が記録される (デバイス種別ごとの接地オフセットを内蔵)。
   持ち上げた瞬間など、速度が閾値を超えたサンプルは自動で棄却される。
